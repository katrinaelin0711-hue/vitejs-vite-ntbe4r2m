...
function parseDatabasePrice(priceVal) {
  ...
  const cleanStr = String(priceVal)
    ...
    .replace(/%/g, '')
    .trim();
  if (cleanStr === '' || cleanStr === '협의' || isNaN(Number(cleanStr)))
    return 0;
...
// match real names against the TABLES constants above.
window.discoverTables = async function () {
  ...
  for (const [key, name] of Object.entries(TABLES)) {
    const { data, error } = await supabaseClient
      ...
      .limit(1);
    console.log(
      `${key.padEnd(14)} -> "${name}"`.padEnd(40),
      error
        ? `MISSING (${error.message})`
        : `OK (sample: ${JSON.stringify(data[0])})`
    );
  }
};

// Same check, but renders directly on the page — no console needed.
// Runs automatically on load (see initializeCalculatorApp) and writes
// a pass/fail line per table into #table-diagnostics.
async function renderTableDiagnostics() {
  const container = document.getElementById('table-diagnostics');
  if (!container || !supabaseClient) return;

  container.innerHTML = '<strong>Table diagnostics:</strong><br>';
  for (const [key, name] of Object.entries(TABLES)) {
    const { data, error } = await supabaseClient
      .from(name)
      .select('*')
    ...
    if (error) {
      ...
      line.textContent = `✗ ${key} -> "${name}" — MISSING (${error.message})`;
    } else {
      line.style.color = '#27ae60';
      line.textContent = `✓ ${key} -> "${name}" — OK`;
    }
...
async function loadAllTables() {
  ...
  dbCache.paper = paper;
  console.log('[DEBUG] paper rows:', paper.length, 'first row:', paper[0]);
  dbCache.size = size;
  ...
  const totalRows = Object.values(dbCache).reduce(
    (sum, arr) => sum + arr.length,
    0
  );
  if (totalRows === 0) {
    setStatus(
      '⚠ No data loaded from Supabase. Open the console and run discoverTables() to debug table names.'
    );
  } else {
...
function populateDropdowns() {
  ...
  fillSelect(document.getElementById('select-size'), dbCache.size, {
    valueKey: 'size',
    labelFn: (r) =>
      `${norm(r.size)}${r.length_width ? ` (${r.length_width}mm)` : ''}`,
  });
  ...
  fillSelect(
    ...
    dbCache.ctpPlateCount,
    {
      ...
      labelFn: (r) =>
        `${norm(r['print color count'])} (${r['plate count']} plates)`,
    }
  );

  // Binding method dropdown (Binding_DB: Binding, English)
  fillSelect(document.getElementById('select-binding'), dbCache.binding, {
    valueKey: 'Binding',
  ...
  fillSelect(document.getElementById('select-coating'), dbCache.coating, {
    valueKey: 'item',
    placeholder: 'None',
  });

  // Epoxy dropdown (PPP_DB_Epoxy: item=에폭시 always, size = sheet size)
  fillSelect(document.getElementById('select-epoxy'), dbCache.epoxy, {
    valueKey: 'size',
    placeholder: 'None',
  });

  // Grounding/folding dropdown (PPP_DB_Grounding: size = fold type)
  fillSelect(document.getElementById('select-grounding'), dbCache.grounding, {
    valueKey: 'size',
    placeholder: 'None',
  });
}
...
function getInputs() {
  return {
    ...
    pageYield:
      parseInt(document.getElementById('input-page-yield').value, 10) || 16,
    quantity:
      parseInt(document.getElementById('input-quantity').value, 10) || 0,
...
function getPaperUnitPrice(paperKey, paperStdSize) {
  ...
  const row = dbCache.paper.find(
    (r) =>
      norm(r.item) === item &&
      norm(r.thickness) === thickness &&
      norm(r.size) === paperStdSize
  );
  if (!row) return 0;
...
// Plate fee comes from CTP_print_DB1 ("인쇄판대" rows); we use the first
// matching plate-fee row for the chosen paper standard size as the per-plate rate.
function calcPlateCost(colorProfile, paperStdSize) {
  ...
  // Find a plate-fee row whose paper size matches; fall back to the first plate fee row available.
  const feeRow =
    dbCache.ctpPlateFee.find(
      (r) =>
        norm(r['paper size']) === paperStdSize && norm(r.item).includes('판대')
    ) || dbCache.ctpPlateFee.find((r) => norm(r.item).includes('판대'));
  const perPlateFee = feeRow ? parseDatabasePrice(feeRow['supply price']) : 0;
...
function parseReamBand(noteStr) {
  ...
  if (overOnlyMatch)
    return { min: parseFloat(overOnlyMatch[1]), max: Infinity };

  // " 5R 이상 ~ 10R 미만 " -> min 5, max 10
  const rangeMatch = note.match(
    /^(\d+(\.\d+)?)R\s*이상\s*~\s*(\d+(\.\d+)?)R\s*미만/
  );
  if (rangeMatch)
...
function runCalculation() {
  ...
  const plateCost = calcPlateCost(input.colorProfile, input.paperStdSize);
  ...
  const dieCutCost = calcDieCuttingCost(
    input.dieCutting,
    input.paperStdSize,
    reams
  );
  const finishingCost = coatingCost + epoxyCost + groundingCost + dieCutCost;
  ...
  document.getElementById('cost-printing').textContent =
    formatKRW(printRunCost);
  document.getElementById('cost-finishing').textContent =
    formatKRW(finishingCost);
...
async function initializeCalculatorApp() {
  if (!initSupabase()) {
    setStatus(
      '⚠ Supabase library did not load. Check the CDN <script> tag in index.html and your network connection.'
    );
    return;
  }
  await renderTableDiagnostics();
  await loadAllTables();
  populateDropdowns();
  attachListeners();
  runCalculation();
